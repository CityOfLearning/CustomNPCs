...
public class BlockCampfire extends BlockLightable {
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumFacing side,
		...
		if (!world.isRemote) {
			...
			if (((item.getItem() == Items.flint) || (item.getItem() == Items.flint_and_steel))
					&& (unlitBlock() == this)) {
			...
			if ((litBlock() == this)) {
				if ((item.getItem() == Item.getItemFromBlock(Blocks.sand))) {
					...
					player.addChatComponentMessage(new ChatComponentTranslation("Set Spawn Point to: %s", pos));
				}
			}
		} else {
			if ((litBlock() == this) && (player.inventory.getCurrentItem() != null)
					&& (player.inventory.getCurrentItem().getItem() == Items.stick)) {
				player.setSpawnPoint(player.getPosition(), true);
			}
		}
		return true;
	...
	public void randomDisplayTick(World world, BlockPos pos, IBlockState state, Random random) {
		// if(world.getLightFor(EnumSkyBlock.SKY, pos) > 7){
		//
		// }
