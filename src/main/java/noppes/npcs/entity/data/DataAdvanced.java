...
public class DataAdvanced implements INPCAdvanced {
   ...
   private String hurtSound = "minecraft:game.player.hurt";
   private String deathSound = "minecraft:game.player.hurt";
   ...
   private Lines getLines(int type) {
      return type == 0?this.interactLines:(type == 1?this.attackLines:(type == 2?this.worldLines:(type == 3?this.killedLines:(type == 4?this.killLines:null))));
   }
   ...
   public void setLine(int type, int slot, String text, String sound) {
      ...
      if(text != null && !text.isEmpty()) {
         Line line = (Line)lines.lines.get(Integer.valueOf(slot));
         line.text = text;
         if(sound != null) {
            line.sound = sound;
         }
      } else {
         lines.lines.remove(Integer.valueOf(slot));
      }

   }

   public String getLine(int type, int slot) {
      Line line = (Line)this.getLines(type).lines.get(Integer.valueOf(slot));
