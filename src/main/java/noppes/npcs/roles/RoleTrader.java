...
public class RoleTrader extends RoleInterface implements IRoleTrader {
   ...
   public IItemStack getSold(int slot) {
      ItemStack item = this.inventorySold.getStackInSlot(slot);
      return item == null?null:new ItemStackWrapper(item);
   }

   public IItemStack getCurrency1(int slot) {
      ItemStack item = this.inventoryCurrency.getStackInSlot(slot);
      return item == null?null:new ItemStackWrapper(item);
   }

   public IItemStack getCurrency2(int slot) {
      ItemStack item = this.inventoryCurrency.getStackInSlot(slot + 18);
      return item == null?null:new ItemStackWrapper(item);
   }
   ...
   public void set(int slot, IItemStack currency, IItemStack currency2, IItemStack sold) {
      if(sold == null) {
         ...
         if(currency != null) {
            this.inventoryCurrency.items.put(Integer.valueOf(slot), currency.getMCItemStack());
         } else {
            this.inventoryCurrency.items.remove(Integer.valueOf(slot));
         }

         if(currency2 != null) {
            this.inventoryCurrency.items.put(Integer.valueOf(slot + 18), currency2.getMCItemStack());
         } else {
            this.inventoryCurrency.items.remove(Integer.valueOf(slot + 18));
         }

         this.inventorySold.items.put(Integer.valueOf(slot), sold.getMCItemStack());
      } else {
   ...
   public void remove(int slot) {
      if(slot < 18 && slot >= 0) {
         this.inventoryCurrency.items.remove(Integer.valueOf(slot));
         this.inventoryCurrency.items.remove(Integer.valueOf(slot + 18));
         this.inventorySold.items.remove(Integer.valueOf(slot));
      } else {
         throw new CustomNPCsException("Invalid slot: " + slot, new Object[0]);
      }
   }
