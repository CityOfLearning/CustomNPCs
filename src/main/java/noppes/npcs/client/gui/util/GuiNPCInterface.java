...
public abstract class GuiNPCInterface extends GuiScreen {
   ...
   private Map buttons;
   private Map topbuttons;
   private Map sidebuttons;
   ...
   public void mouseClicked(int i, int j, int k) {
      if(AWTWindow == null) {
         if(this.subgui != null) {
            ...
            while(var4.hasNext()) {
               GuiNpcTextField guibutton = (GuiNpcTextField)var4.next();
               if(guibutton.enabled) {
                  guibutton.mouseClicked(i, j, k);
               }
            }
            ...
            if(k == 0) {
               ...
               while(var4.hasNext()) {
                  GuiCustomScroll guibutton1 = (GuiCustomScroll)var4.next();
                  guibutton1.mouseClicked(i, j, k);
               }

               var4 = this.buttonList.iterator();

               while(var4.hasNext()) {
                  GuiButton guibutton2 = (GuiButton)var4.next();
                  if(guibutton2.mousePressed(this.mc, this.mouseX, this.mouseY)) {
                     Pre event = new Pre(this, guibutton2, this.buttonList);
                     if(!MinecraftForge.EVENT_BUS.post(event)) {
                        guibutton2 = event.button;
   ...
   public void drawScreen(int i, int j, float f) {
      ...
      while(var4.hasNext()) {
         ...
         gui.drawLabel(this, this.fontRendererObj);
      }
      ...
      while(var4.hasNext()) {
         ...
         gui2.drawScreen(i, j, f, !this.hasSubGui() && gui2.isMouseOver(i, j)?Mouse.getDWheel():0);
      }

      var4 = (new ArrayList(this.extra.values())).iterator();
   ...
   public void drawNpc(EntityLivingBase entity, int x, int y, float zoomed, int rotation) {
      ...
      this.mc.getRenderManager().renderEntityWithPosYaw(entity, 0.0D, 0.0D, 0.0D, 0.0F, 1.0F);
