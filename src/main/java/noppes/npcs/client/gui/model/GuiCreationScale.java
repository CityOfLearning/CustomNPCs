...
public class GuiCreationScale extends GuiCreationScreenInterface implements ISliderListener, ICustomScrollListener {
   ...
   public void initGui() {
      ...
      this.data.clear();
      EnumParts[] config = parts;
      int y = parts.length;

      for(int var5 = 0; var5 < y; ++var5) {
         EnumParts part = config[var5];
         ModelPartConfig config1;
         if(part == EnumParts.ARM_RIGHT) {
            config1 = this.playerdata.getPartConfig(EnumParts.ARM_LEFT);
            if(!config1.notShared) {
               continue;
            }
         }

         if(part == EnumParts.LEG_RIGHT) {
            config1 = this.playerdata.getPartConfig(EnumParts.LEG_LEFT);
            if(!config1.notShared) {
               continue;
            }
         }
      ...
      this.scroll.setSelected(StatCollector.translateToLocal("part." + selected.name));
      this.scroll.guiLeft = this.guiLeft;
      this.scroll.guiTop = this.guiTop + 46;
      this.scroll.setSize(100, this.ySize - 74);
      this.addScroll(this.scroll);
      ModelPartConfig var8 = this.playerdata.getPartConfig(selected);
      y = this.guiTop + 65;
      this.addLabel(new GuiNpcLabel(10, "scale.width", this.guiLeft + 102, y + 5, 16777215));
      this.addSlider(new GuiNpcSlider(this, 10, this.guiLeft + 150, y, 100, 20, var8.scaleX - 0.5F));
      y += 22;
      this.addLabel(new GuiNpcLabel(11, "scale.height", this.guiLeft + 102, y + 5, 16777215));
      this.addSlider(new GuiNpcSlider(this, 11, this.guiLeft + 150, y, 100, 20, var8.scaleY - 0.5F));
      y += 22;
      this.addLabel(new GuiNpcLabel(12, "scale.depth", this.guiLeft + 102, y + 5, 16777215));
      this.addSlider(new GuiNpcSlider(this, 12, this.guiLeft + 150, y, 100, 20, var8.scaleZ - 0.5F));
      if(selected == EnumParts.ARM_LEFT || selected == EnumParts.LEG_LEFT) {
         ...
         this.addButton(new GuiNpcButton(13, this.guiLeft + 150, y, 50, 20, new String[]{"gui.no", "gui.yes"}, var8.notShared?0:1));
      }
