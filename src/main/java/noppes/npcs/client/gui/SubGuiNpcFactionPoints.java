...
public class SubGuiNpcFactionPoints extends SubGuiInterface implements ITextfieldListener {
	...
	public void initGui() {
		...
		addTextField(new GuiNpcTextField(2, this, guiLeft + 8 + fontRendererObj.getStringWidth(getLabel(2).label),
				guiTop + 28, 70, 20, faction.defaultPoints + ""));
		getTextField(2).setMaxStringLength(6);
		getTextField(2).numbersOnly = true;
		String title = StatCollector.translateToLocal("faction.unfriendly") + "<->"
				+ StatCollector.translateToLocal("faction.neutral");
		addLabel(new GuiNpcLabel(3, title, guiLeft + 4, guiTop + 80));
		addTextField(new GuiNpcTextField(3, this, guiLeft + 8 + fontRendererObj.getStringWidth(title), guiTop + 75, 70,
				20, faction.neutralPoints + ""));
		title = StatCollector.translateToLocal("faction.neutral") + "<->"
				+ StatCollector.translateToLocal("faction.friendly");
		addLabel(new GuiNpcLabel(4, title, guiLeft + 4, guiTop + 105));
		addTextField(new GuiNpcTextField(4, this, guiLeft + 8 + fontRendererObj.getStringWidth(title), guiTop + 100, 70,
				20, faction.friendlyPoints + ""));
