...
public class SubGuiNpcFactionOptions extends SubGuiInterface implements IScrollData, ICustomScrollListener {
	...
	public void initGui() {
		...
		if (scrollFactions == null) {
			(scrollFactions = new GuiCustomScroll(this, 0)).setSize(120, 208);
		}
		...
		if (data.containsValue(options.factionId)) {
			...
			label = label + " " + options.factionPoints + " " + StatCollector.translateToLocal("faction.points");
		...
		if (data.containsValue(options.faction2Id)) {
			...
			label = label + " " + options.faction2Points + " " + StatCollector.translateToLocal("faction.points");
			addLabel(new GuiNpcLabel(6, label, guiLeft + 12, guiTop + 44));
			addButton(new GuiNpcButton(1, guiLeft + 110, guiTop + 35, 20, 20, "X"));
		}
		if ((selected >= 0) && (!data.containsValue(options.faction2Id) || !data.containsValue(options.factionId))
				&& !options.hasFaction(selected)) {
			addButton(new GuiNpcButton(2, guiLeft + 4, guiTop + 60, 90, 20,
