...
public class GuiScriptInterface extends GuiNPCInterface implements IGuiData, GuiYesNoCallback, IJTextAreaListener {
   ...
   public void initGui() {
      ...
      int left;
      for(left = 0; left < this.handler.getScripts().size(); ++left) {
         ScriptContainer var10000 = (ScriptContainer)this.handler.getScripts().get(left);
         this.addTopButton(top = new GuiMenuTopButton(left + 1, top, left + 1 + ""));
      }
      ...
      top.active = true;
      if(this.activeTab > 0) {
         ScriptContainer var6 = (ScriptContainer)this.handler.getScripts().get(this.activeTab - 1);
         this.addTextField(new GuiNpcTextArea(2, this, this.guiLeft + 1 + yoffset, this.guiTop + yoffset, this.xSize - 108 - yoffset, (int)((double)this.ySize * 0.96D) - yoffset * 2, var6 == null?"":var6.script));
         ...
         this.addButton(new GuiNpcButton(108, left1, this.guiTop + 43 + yoffset, 80, 20, "gui.editor"));
         ...
         if(var6 != null) {
            scroll.setList(var6.scripts);
         }

         this.addScroll(scroll);
      } else {
         this.addTextField(new GuiNpcTextArea(2, this, this.guiLeft + 4 + yoffset, this.guiTop + 6 + yoffset, this.xSize - 160 - yoffset, (int)((float)this.ySize * 0.92F) - yoffset * 2, this.getConsoleText()));
         this.getTextField(2).canEdit = false;
         left = this.guiLeft + this.xSize - 150;
   ...
   private String getConsoleText() {
      ...
      int tab = 0;
      Iterator console = this.handler.getScripts().iterator();

      while(console.hasNext()) {
         ScriptContainer script = (ScriptContainer)console.next();
         ++tab;
         Iterator entry = script.console.entrySet().iterator();

         while(entry.hasNext()) {
            Entry entry1 = (Entry)entry.next();
            map.put(entry1.getKey(), " tab " + tab + ":\n" + (String)entry1.getValue());
         }
      }
      ...
      Entry var9;
      for(Iterator var8 = map.entrySet().iterator(); var8.hasNext(); var7 = new Date(((Long)var9.getKey()).longValue()) + (String)var9.getValue() + "\n" + var7) {
   ...
   protected void actionPerformed(GuiButton guibutton) {
      ...
      if(guibutton.id == 102) {
         if(this.activeTab > 0) {
            ...
            Iterator container1 = this.handler.getScripts().iterator();

            while(container1.hasNext()) {
               ScriptContainer script = (ScriptContainer)container1.next();
      ...
      if(guibutton.id == 105) {
         GuiYesNo container2 = new GuiYesNo(this, "Confirm", StatCollector.translateToLocal("gui.delete"), 10);
         this.displayGuiScreen(container2);
      }
