...
public class RenderChatMessages implements IChatMessages {
   ...
   public void addMessage(String message, EntityNPCInterface npc) {
      if(CustomNpcs.EnableChatBubbles) {
         ...
         if(!message.equals(this.lastMessage) || this.lastMessageTime + 5000L <= time) {
            ...
            messages.put(Long.valueOf(time), new TextBlockClient(message, this.boxLength * 4, true, new Object[]{Minecraft.getMinecraft().thePlayer, npc}));
            if(messages.size() > 3) {
   ...
   private void render(double par3, double par5, double par7, float textscale, boolean depth) {
      ...
      TextBlockClient textYSize;
      for(Iterator mc = this.messages.values().iterator(); mc.hasNext(); size += textYSize.lines.size()) {
         textYSize = (TextBlockClient)mc.next();
      }

      Minecraft var23 = Minecraft.getMinecraft();
      int var24 = (int)((float)(size * font.FONT_HEIGHT) * this.scale);
      GlStateManager.translate((float)par3 + 0.0F, (float)par5 + (float)var24 * textscale * var14, (float)par7);
      GlStateManager.scale(textscale, textscale, textscale);
      GL11.glNormal3f(0.0F, 1.0F, 0.0F);
      GlStateManager.rotate(-var23.getRenderManager().playerViewY, 0.0F, 1.0F, 0.0F);
      GlStateManager.rotate(var23.getRenderManager().playerViewX, 1.0F, 0.0F, 0.0F);
      ...
      this.drawRect(-this.boxLength - 2, -2, this.boxLength + 2, var24 + 1, white, 0.11D);
      this.drawRect(-this.boxLength - 1, -3, this.boxLength + 1, -2, black, 0.1D);
      this.drawRect(-this.boxLength - 1, var24 + 2, -1, var24 + 1, black, 0.1D);
      this.drawRect(3, var24 + 2, this.boxLength + 1, var24 + 1, black, 0.1D);
      this.drawRect(-this.boxLength - 3, -1, -this.boxLength - 2, var24, black, 0.1D);
      this.drawRect(this.boxLength + 3, -1, this.boxLength + 2, var24, black, 0.1D);
      this.drawRect(-this.boxLength - 2, -2, -this.boxLength - 1, -1, black, 0.1D);
      this.drawRect(this.boxLength + 2, -2, this.boxLength + 1, -1, black, 0.1D);
      this.drawRect(-this.boxLength - 2, var24 + 1, -this.boxLength - 1, var24, black, 0.1D);
      this.drawRect(this.boxLength + 2, var24 + 1, this.boxLength + 1, var24, black, 0.1D);
      this.drawRect(0, var24 + 1, 3, var24 + 4, white, 0.11D);
      this.drawRect(-1, var24 + 4, 1, var24 + 5, white, 0.11D);
      this.drawRect(-1, var24 + 1, 0, var24 + 4, black, 0.1D);
      this.drawRect(3, var24 + 1, 4, var24 + 3, black, 0.1D);
      this.drawRect(2, var24 + 3, 3, var24 + 4, black, 0.1D);
      this.drawRect(1, var24 + 4, 2, var24 + 5, black, 0.1D);
      this.drawRect(-2, var24 + 4, -1, var24 + 5, black, 0.1D);
      this.drawRect(-2, var24 + 5, 1, var24 + 6, black, 0.1D);
      ...
      while(var18.hasNext()) {
         ...
         for(Iterator var20 = block.lines.iterator(); var20.hasNext(); ++index) {
            IChatComponent chat = (IChatComponent)var20.next();
