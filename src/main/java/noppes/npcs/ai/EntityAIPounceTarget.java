...
public class EntityAIPounceTarget extends EntityAIBase {
   ...
   public boolean shouldExecute() {
      if(!this.npc.onGround) {
         ...
         return this.leapTarget != null && this.npc.getEntitySenses().canSee(this.leapTarget)?(!this.npc.isInRange(this.leapTarget, 4.0D) && this.npc.isInRange(this.leapTarget, 8.0D)?this.npc.getRNG().nextInt(5) == 0:false):false;
      }
   }
