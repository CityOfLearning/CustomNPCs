...
public class EntityAIClosestTarget extends EntityAITarget {
   ...
   public EntityAIClosestTarget(EntityCreature par1EntityCreature, Class par2Class, int par3, boolean par4, boolean par5) {
      this(par1EntityCreature, par2Class, par3, par4, par5, (Predicate)null);
   }

   public EntityAIClosestTarget(EntityCreature par1EntityCreature, Class par2Class, int par3, boolean par4, boolean par5, Predicate par6IEntitySelector) {
   ...
   public boolean shouldExecute() {
      if(this.targetChance > 0 && this.taskOwner.getRNG().nextInt(this.targetChance) != 0) {
         ...
         List list = this.taskOwner.worldObj.getEntitiesWithinAABB(this.targetClass, this.taskOwner.getEntityBoundingBox().expand(d0, (double)MathHelper.ceiling_double_int(d0 / 2.0D), d0), this.targetEntitySelector);
