...
public class FlyNodeProcessor extends NodeProcessor {
   ...
   public PathPoint getPathPointTo(Entity p_176161_1_) {
      return this.openPoint(MathHelper.floor_double(p_176161_1_.getEntityBoundingBox().minX), MathHelper.floor_double(p_176161_1_.getEntityBoundingBox().minY + 0.5D), MathHelper.floor_double(p_176161_1_.getEntityBoundingBox().minZ));
   }
      ...
      for(int i = x; i < x + this.entitySizeX; ++i) {
         for(int j = y; j < y + this.entitySizeY; ++j) {
            for(int k = z; k < z + this.entitySizeZ; ++k) {
               ...
               if((block.getMaterial() != Material.water || !(entityIn instanceof EntityNPCInterface) || !((EntityNPCInterface)entityIn).ai.canSwim) && block.getMaterial() != Material.air) {
                  return 0;
               }
            }
         }
